    ...
    sti();
	  ...
	  if(what_scheduler == 1){
		  ...
		  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		      ...
		      smallest_proc_pass  -> pass =   smallest_proc_pass -> pass + smallest_proc_pass -> stride;
		      ran = 1;
		          c->proc =   smallest_proc_pass ;
		      
		         switchuvm(smallest_proc_pass );
		      smallest_proc_pass -> state = RUNNING;
